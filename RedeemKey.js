...
form.addEventListener("submit", e => {
  ...
  if (existingKey) {
    ...
    // Key has already been redeemed by a different user
      ...
      const existingUser = loginUsers.find(user => user.username === existingKey.redeemedBy);
      alert(`This key has already been redeemed by ${existingUser.name} (${existingUser.username}).`);
    }
  } 
